...
export default class EventHubManager {
    ...
    */
    async registerBlockEvent(onEvent: Function, onError?: Function): Promise<number> {
        const handle = this.eventHub.registerBlockEvent(onEvent, onError)
    ...
    */
    async registerChaincodeEvent(
        ...
        const handle = this.eventHub.registerChaincodeEvent(ccId, eventName, onEvent, onError)
    ...
    * @param [onError] - Optional callback function to be notified when this event hub is shutdown. The shutdown may be caused by a network error or by a call to the "disconnect()" method or a connection error.
    */
    async registerTxEvent(txId: string, onEvent: Function, onError: Function): Promise<string> {
        this.eventHub.registerTxEvent(txId, onEvent, onError)
        const currNumber = this.txEventMap.get(txId)
        if (typeof currNumber === 'undefined') {
            this.chaincodeEventMap.set(txId, 1)
        } else {
            this.chaincodeEventMap.set(txId, currNumber + 1)
        }
        if (!this.eventHub.isconnected()) {
            this.eventHub.connect()
            await this.waitEventHubConnected()
    ...
    */
    unregisterTxEvent(txId: string) {
        ...
        const currNumber = this.chaincodeEventMap.get(txId)
        ...
        if (typeof currNumber === 'undefined') {
            ...
            this.chaincodeEventMap.delete(txId)
        }
