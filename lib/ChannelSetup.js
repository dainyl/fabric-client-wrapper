...
export default class ChannelSetup {
    ...
    async distributedServerRun(channel: Channel, network: NetworkOpts): Promise<Channel> {
        const distributedSetupChannelServer = new ChannelSetupServer({
        ...
        if (this.createChannelOp) {
            ...
            initChannelPromise = initChannelPromise.then(() => createChannelOp(channel)).then(genesisBlock => {
                requestResponsesPromise = distributedSetupChannelServer.requestResponses(network.timeout)
                return genesisBlock
            })
        } else {
            requestResponsesPromise = distributedSetupChannelServer.requestResponses(network.timeout)
    ...
    async distributedClientRun(channel: Channel, network: NetworkOpts): Promise<Channel> {
        if (!network.host) {
            throw new Error('Error: network.host is required for client')
        }
        const distributedSetupChannelClient = new ChannelSetupClient({
            mspIds: network.mspIds || [(this.userClient: any).getMspId()],
            host: network.host,
            port: network.port,
        })
        ...
        await distributedSetupChannelClient.waitRequest(network.timeout)
    ...
    async run(): Promise<Channel> {
        ...
        if (this.network) {
            if (this.network.leader) {
                return this.distributedServerRun(channel, this.network)
            }
            return this.distributedClientRun(channel, this.network)
        }
