...
import newChannel, { type ChannelOpts } from "./newChannel"
...
export type NetworkOpts = {
    ...
    timeout?: number,
    onError?: Function,
    channelSetupClient?: ChannelSetupClient,
    channelSetupServer?: ChannelSetupServer
}
 ...
 * @param channelOrChannelOpts - Either the EventHubChannel object you wish to use or the arguments to create a new EventHubChannel
...
export default class ChannelSetup {
    ...
    constructor(
        userClient: UserClient,
        channelOrChannelOpts: Channel | ChannelOpts,
        ...
        if ("getPeers" in channelOrChannelOpts) {
            ...
            this.newChannelOp = () => newChannel(channelOrChannelOpts)
        }
    ...
    async serverRun(channel: Channel, network: NetworkOpts): Promise<Channel> {
        const channelSetupServer =
            network.channelSetupServer ||
            new ChannelSetupServer({
                port: network.port
            })
        const channelName = channel.getName()
        channelSetupServer.connectChannel({
        ...
        if (this.createChannelOp) {
            ...
            initChannelPromise = initChannelPromise
                ...
                .then(genesisBlock => {
                    requestResponsesPromise = channelSetupServer.requestClientInit(
                        channelName,
                        network.timeout
                    )
                    return genesisBlock
        ...
        channelSetupServer.done(channelName)
        if (!network.channelSetupServer) {
    ...
    async clientRun(channel: Channel, network: NetworkOpts): Promise<Channel> {
        ...
        if (network.channelSetupClient) {
            channelSetupClient = network.channelSetupClient
        } else {
            if (!network.host) {
                throw new Error("Error: network.host is required for client")
            }
            channelSetupClient = new ChannelSetupClient({
                host: network.host,
                port: network.port,
        ...
        await channelSetupClient.waitClientInitRequest(
            channelName,
            network.timeout
        )
        ...
        channelSetupClient.sendClientDone(channelName)
        await channelSetupClient.waitServerDone(channelName, network.timeout)
