...
export default class ChannelSetup {
    ...
    async serverRun(channel: Channel, network: NetworkOpts): Promise<Channel> {
        ...
        if (this.createChannelOp) {
            ...
            initChannelPromise = initChannelPromise.then(() => createChannelOp(channel)).then(genesisBlock => {
                requestResponsesPromise = channelSetupServer.requestResponses(network.timeout)
                return genesisBlock
            })
        } else {
            requestResponsesPromise = channelSetupServer.requestResponses(network.timeout)
        ...
        channelSetupServer.sendCompleted()
        return channel
    }
    ...
    async clientRun(channel: Channel, network: NetworkOpts): Promise<Channel> {
        ...
        channelSetupClient.sendDone()
        await channelSetupClient.waitCompleted(network.timeout)
