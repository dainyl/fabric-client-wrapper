...
import type UserClient, { CreateChannelRequest, JoinChannelRequest, TimeoutPeersOpts } from './UserClient'
...
export default class ChannelSetup {
    ...
    */
    withCreateChannel(
        ...
        if (this.isSwallowAlreadyCreatedErrors(swallowAlreadyCreatedErrors)) {
            ...
            this.createChannelOp = (channel: Channel) =>
                this.userClient.createChannel(channel, createChannelRequest, timeout).then(response => response.wait())
        }
        return this
    }
    ...
    * @returns The ChannelSetup instance
    */
    withUpdateChannel(updateChannelRequest: CreateChannelRequest, timeout?: number): ChannelSetup {
        ...
        this.updateChannelOp = (channel: Channel) =>
            this.userClient.updateChannel(channel, updateChannelRequest, timeout).then(response => response.wait())
        return this
    }
    ...
    */
    withInstantiateChaincode(
        ...
        this.instantiateChaincodeOps.push(channel =>
            this.userClient
                .instantiateChaincode(channel, chaincodeInstantiateRequest, timeoutPeersOpts)
                .then(response => response.wait())
                .catch(error => {
                    if (
                        !this.isSwallowAlreadyCreatedErrors(swallowAlreadyCreatedErrors) ||
    ...
    */
    withUpgradeChaincode(
        ...
        this.upgradeChaincodeOps.push(channel =>
            this.userClient
                .upgradeChaincode(channel, chaincodeUpgradeRequest, timeoutPeersOpts)
                .then(response => response.wait())
        )
        return this
    }
