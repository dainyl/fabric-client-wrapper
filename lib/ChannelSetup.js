...
import { createFcwChannel, type CreateFcwChannelOpts } from './fabric-client-extended/FcwChannel'
import { isFcwPeer } from './fabric-client-extended/FcwPeer'
import DistributedSetupChannelServer from './distributed/DistributedSetupChannelServer'
import DistributedSetupChannelClient from './distributed/DistributedSetupChannelClient'
...
export default class ChannelSetup {
    ...
    constructor(
        ...
        channelOrChannelOpts: Channel | CreateFcwChannelOpts,
        { swallowAlreadyCreatedErrors, distributed }: ChannelSetupOpts = {}
    ) {
        ...
        if ('getPeers' in channelOrChannelOpts) {
            ...
            this.newChannelOp = () => createFcwChannel(channelOrChannelOpts)
        }
        ...
        this.installChaincodeOps.push(channel => {
            ...
            if (chaincodeInstallRequest.targets) {
                ...
                targets = channel
                    .getPeers()
                    .filter(
                        peer => !isFcwPeer(peer) || peer.getAdminMspIds().includes((this.userClient: any).getMspId())
                    )
            }
    ...
    async distributedServerRun(channel: Channel, distributed: DistributedOpts): Promise<Channel> {
        const distributedSetupChannelServer = new DistributedSetupChannelServer({
            userClient: (this.userClient: any),
            channel,
            externalMspIds: distributed.externalMspIds,
        ...
        if (this.createChannelOp) {
            const createChannelOp = this.createChannelOp
            initChannelPromise = initChannelPromise.then(() => createChannelOp(channel)).then(genesisBlock => {
                requestResponsesPromise = distributedSetupChannelServer.requestResponses(distributed.timeout)
                return genesisBlock
            })
        } else {
            requestResponsesPromise = distributedSetupChannelServer.requestResponses(distributed.timeout)
        ...
        distributedSetupChannelServer.sendCompleted()
        return channel
    }

    async distributedClientRun(channel: Channel, distributed: DistributedOpts): Promise<Channel> {
        if (!distributed.host) {
            throw new Error('Error: distributed.host is required for client')
        }
        const distributedSetupChannelClient = new DistributedSetupChannelClient({
            mspIds: distributed.mspIds || [(this.userClient: any).getMspId()],
        ...
        await distributedSetupChannelClient.waitRequest(distributed.timeout)
        ...
        await [installChaincodePromise, joinChannelPromise]

        distributedSetupChannelClient.sendDone()
        await distributedSetupChannelClient.waitCompleted()
    ...
    async run(): Promise<Channel> {
        ...
        if (this.distributed) {
            if (this.distributed.leader) {
                return this.distributedServerRun(channel, this.distributed)
            }
            return this.distributedClientRun(channel, this.distributed)
        }
