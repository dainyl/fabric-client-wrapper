...
export default function pickPeersForPolicy(
    ...
    const identities = _.clone(policy.identities)
    if (remainingKeys.length !== 0) {
        const mspIdOrder = {}
        policy.identities.forEach(({ role: { mspId } }, index) => {
            mspIdOrder[mspId] = index
        })
        remainingKeys.sort((a, b) => {
            const [aMspId] = a.split("_")
            const [bMspId] = b.split("_")
            const aOrder = mspIdOrder[aMspId]
            const bOrder = mspIdOrder[bMspId]
