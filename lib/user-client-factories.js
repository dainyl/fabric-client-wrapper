...
* @param {OrganizationConfig} [opts.organizationConfig] - The OrganizationConfig to use, Note only used if UserClient is not specfied
...
export async function createUserClientFromStore(
    ...
    let usedOrganizationConfig
    if (userClient) {
        const user = await userClient.client.loadUserFromStateStore(username)
        await fabricClient.setUserContext(user)
        fabricClient.setStateStore(userClient.organizationConfig.store)
        usedOrganizationConfig = userClient.organizationConfig
    } else if (organizationConfig) {
        fabricClient.setStateStore(organizationConfig.store)
        await fabricClient.getUserContext(username, true)
        usedOrganizationConfig = organizationConfig
    } else {
        throw new Error('create UserClient From Store requires either userClient or organizationConfig')
    }

    return new UserClient(fabricClient, usedOrganizationConfig, roles)
}
