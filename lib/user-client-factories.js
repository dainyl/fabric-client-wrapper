...
export async function createUserClientFromKeys({
    ...
    const key = await cryptoSuite.importKey(userKeyConfig.cryptoContent.privateKeyPEM, {
...
export async function createUserClientFromCAEnroll({
    ...
    await addUserToClient(client, username, mspId, cryptoSuite, store, enrollment.key, enrollment.certificate)
...
export async function createUserClientFromCARegisterAndEnroll({
    ...
    const secret = await userClient.registerUserInCA(username, affiliation, registerOpts)
...
export async function createUserClientFromStore({
    ...
    if (!(cryptoSuite && store) && !userClient) {
        throw new Error('createUserClientFromStore requires either store+cryptoSuite or userClient')
    }
