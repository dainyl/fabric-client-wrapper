...
async function addUserToClient(client, username, organizationConfig, key, certificate) {
    const user = new User(username)
    if (organizationConfig.cryptoSuite) {
        user.setCryptoSuite(client.getCryptoSuite())
    }
    await user.setEnrollment(key, certificate, organizationConfig.mspId)
    return client.setUserContext(user, !organizationConfig.store)
}
/**
...
export async function createUserClientFromKeys(
    ...
    if (organizationConfig.store) {
        client.setStateStore(organizationConfig.store)
    }

    const key = await organizationConfig.cryptoSuite.importKey(userKeyConfig.cryptoContent.privateKeyPEM, {
        ephemeral: !organizationConfig.cryptoSuite._cryptoKeyStore, // eslint-disable-line no-underscore-dangle
    })
...
export async function createUserClientFromCAEnroll(
    ...
    await addUserToClient(client, username, organizationConfig, enrollment.key, enrollment.certificate)
...
export async function createUserClientFromStore(
    ...
    if (userClient) {
        ...
        await fabricClient.setUserContext(user, !orgConfig.store)
    } else {
