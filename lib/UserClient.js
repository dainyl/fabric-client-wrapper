...
function createSendTransactionWait(channel, peers, transactionId, waitEventPromise, timeout, pollInterval) {
    return () => {
...
export default class UserClient {
    ...
    */
    async isChannelJoined(channel: Channel): Promise<boolean> {
        ...
        if (peers.length === 0) {
            throw new Error(
                `Error cannot call isChannelJoined, channel contains no peers that are owned by this user's Organisation ${this
                    .organizationConfig.mspId}`
            )
        }
    ...
    */
    async isChaincodeInstantiated(channel: Channel, chaincodeId: string, chaincodeVersion?: string): Promise<boolean> {
        const target = channel
            .getPeers()
            .find(peer => isFcwPeer(peer) && peer.getAdminMspIds().includes(this.organizationConfig.mspId))
        if (!target) {
            throw new Error(
                `Error cannot call isChaincodeInstantiated, channel contains no peers that are owned by this user's Organisation ${this
                    .organizationConfig.mspId}`
            )
        }
    ...
    */
    async instantiateUpgradeChaincode(
        ...
        { waitPeers, proposalTimeout, waitTimeout, pollInterval }: TimeoutPeersOpts = {
            waitPeers: undefined,
    ...
    * @param {number} [timeoutPeersOpts.pollInterval=1000] - A number indicating milliseconds to wait before polling peers without Eventhubs
    */
    ...
    */
    async sendTransaction(
        ...
        transactionRequest: TransactionRequest,
        { waitPeers, waitTimeout, pollInterval }: TimeoutPeersOpts = {
            waitPeers: undefined,
            waitTimeout: 600000,
            pollInterval: 1000,
        }
