                ...
                .map(
                    eventHubManager =>
                        new Promise(async (resolve, reject) => {
                            ...
                            blockRegistrationNumber = await eventHubManager.registerBlockEvent(
                                block => {
                                    clearTimeout(handle)
                                    eventHubManager.unregisterBlockEvent(blockRegistrationNumber)
...
export default class UserClient {
    ...
    */
    async joinChannel(
        channel: Channel,
        { targets, genesisBlock, timeout = 60000 }: JoinChannelOpts = {}
    ): Promise<{
        data: Array<Object>,
        wait: () => Promise<any>,
    }> {
        ...
        await peers.filter(peer => isEventHubPeer(peer)).map(peer => peer.getEventHubManager().waitEventHubConnected())
    ...
    */
    async instantiateUpgradeChaincode(
        ...
        if (isFcwChannel(channel)) {
            channel.getTransactionIdTimeMap().set(transactionId)
        }

        const data = await channel.sendTransaction(transactionRequest)

        return {
            data,
            wait: () => {
                const waitExternalPeers = fullWaitTransactionPeers.filter(peer => !isEventHubPeer(peer))
                let endDate = new Date()
                ...
                return Promise.all([waitInternalPromise, waitExternalPromise])
            },
        }
    }
    ...
    */
    ...
    queryChaincode(
        ...
        return this.sendTransactionProposal(channel, chaincodeId, targets, opts).then(response => ({
            data: formatProposalResponse(response.data.transactionRequest.proposalResponses[0].response),
        }))
    }
