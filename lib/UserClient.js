...
export default class UserClient {
    ...
    createBlockEventPromise(
        ...
        return Promise.race(
            targets.filter(peer => peer.getMspId() === this.getMspId()).map(
                peer =>
    ...
    async updateChannel(
        ...
        if (disable || waitPeers.length !== 0) {
            ...
            await waitPeers.map(peer =>
                peer.getEventHubManager().waitEventHubConnected()
            )
        }
    ...
    async joinChannel(
        ...
        return {
            ...
            wait: (waitOpts = {}) => {
                ...
                return this.createJoinChannelWait(
                    userChannel,
                    waitPeers,
                    waitOpts.timeout
                )
    ...
    async instantiateUpgradeChaincode(
        ...
        const txId =
            chaincodeInstantiateUpgradeRequest.txId || this.newTransactionID()
