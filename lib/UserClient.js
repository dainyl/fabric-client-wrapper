...
import {
    MEMBER_ROLE,
    type FcwChaincodeInstantiateUpgradeRequest
} from "./shared"
...
function createSendTransactionWait(
    channel,
    defaultPeers,
    transactionId,
 ...
 * @param {Client} opts.client - The FabricClient object to wrap
...
export default class UserClient {
     ...
     */
    async registerUserInCA(
        username: string,
        affiliation: string,
        opts: CARegisterOpts = {}
    ): Promise<string> {
        // TODO update to match fabric-client format
    ...
    async createChannel(
        ...
        data: Object,
        wait: (
            { timeout: ?number, pollInterval?: number } | void
        ) => Promise<Object>
    }> {
                ...
                peer =>
                    new Promise(async (resolve, reject) => {
                        ...
                        blockRegistrationNumber = eventHubManager.registerBlockEvent(
                            block => {
                                ...
                                if (block.data.data.length === 1) {
                                    const channelHeader =
                                        block.data.data[0].payload.header
                                            .channel_header
                        ...
                        if (
                            response.channels.some(
                                channelInfo =>
                                    channelInfo.channel_id === channel.getName()
                            )
                        ) {
                            clearTimeout(handle)
                            eventHubManager.unregisterBlockEvent(
    ...
    async instantiateUpgradeChaincode(
        ...
        if (
            !chaincodeInstantiateUpgradeRequest.targets &&
            chaincodeInstantiateUpgradeRequest.targetsPolicy
        ) {
            const targetsPolicy =
                chaincodeInstantiateUpgradeRequest.targetsPolicy
            chaincodeInstantiateUpgradeRequest.targets = pickPeersForPolicy(
                userChannel.getPeers(),
                targetsPolicy
            )
        }
        ...
        const [proposalResponses, proposal] = await (upgrade
            ? userChannel.sendUpgradeProposal(
                  fullInstantiateUpgradeRequest,
                  timeout
              )
            : userChannel.sendInstantiateProposal(
                  fullInstantiateUpgradeRequest,
                  timeout
              ))
    ...
    async sendTransactionProposal(
        ...
        const [
            proposalResponses,
            proposal
        ] = await userChannel.sendTransactionProposal(request)
