...
function createBlockEventPromise(channel: Channel, peers: Array<Peer>, timeout: number) {
    ...
    return [
        Promise.race(
            peers
                .filter(peer => peer.getEventHub)
                .map(peer => peer.getEventHub())
                .map(
                    eventHub =>
                        new Promise((resolve, reject) => {
                            ...
                            blockRegistrationNumber = eventHub.registerBlockEvent(
                                block => {
                                    clearTimeout(handle)
                                    eventHub.unregisterBlockEvent(blockRegistrationNumber)
                                ...
                                () => {
                                    clearTimeout(handle)
                                    eventHub.unregisterTxEvent(blockRegistrationNumber)
                                }
                            )
...
export default class UserClient {
    ...
    */
    async instantiateUpgradeChaincode(
        ...
        const waitInternalPeers = fullWaitTransactionPeers.filter(peer => typeof peer.getEventHub !== 'undefined')
        const waitInternalPromise = Promise.all(
            waitInternalPeers.map(peer => peer.getEventHub()).map(
                eventHub =>
                    new Promise((resolve, reject) => {
                        const handle = setTimeout(() => {
                            eventHub.unregisterTxEvent(instantiateId)
                            reject()
                        }, waitTransactionPeersTimeout)

                        eventHub.registerTxEvent(instantiateId, (tx, code) => {
                            clearTimeout(handle)
                            eventHub.unregisterTxEvent(instantiateId)
                            if (code !== 'VALID') {
                                reject()
        ...
        const waitExternalPeers = fullWaitTransactionPeers.filter(peer => typeof peer.getEventHub === 'undefined')
        let endDate = new Date()
