...
import { createEventHubPeer, isEventHubPeer } from './fabric-client-extended/EventHubPeer'
...
async function waitChaincodeRunningPeer(userClient, channel, peer, chaincodeId, timeout, pollInterval) {
    ...
    while (new Date() < endDate) {
        try {
            ...
            if (!error.message || !error.message.includes('premature execution')) {
                return true
            }
...
function createSendTransactionWait(channel, defaultPeers, transactionId, instantiateChaincodeId, userClient: any) {
    return (
...
export default class UserClient {
     ...
     * @ignore
     */
    createBlockEventPromise(channel: Channel, peers: Array<Peer>, timeout: number = 60000): Promise<void> {
        return Promise.race(
    ...
    async instantiateUpgradeChaincode(
        ...
        return {
            ...
            wait: createSendTransactionWait(
                ...
                transactionId,
                upgrade ? false : chaincodeInstantiateUpgradeRequest.chaincodeId,
                this
            ),
        }
    }
