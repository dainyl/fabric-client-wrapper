...
function createSendTransactionWait(channel, peers, transactionId) {
    return (
        { race, pollInterval, timeout } = {
            race: false,
            pollInterval: 1000,
            timeout: 60000,
        }
    ) => {
        const eventHubPeersPromises = peers
            .filter(peer => isEventHubPeer(peer))
            .map(peer => waitTransactionEventHubPeer(channel, peer, transactionId, timeout))
        const pollingPeersPromises = peers
            .filter(peer => !isEventHubPeer(peer))
...
export default class UserClient {
    ...
    */
    async createChannel(
        ...
        return {
            data,
            wait: async ({ timeout, pollInterval } = { timeout: 60000, pollInterval: 1000 }) => {
                let endDate = new Date()
                endDate = new Date(endDate.getTime() + timeout)
                while (new Date() < endDate) {
                    try {
    ...
    */
    async sendTransaction(
        ...
        return {
            data,
            wait: createSendTransactionWait(channel, channel.getPeers(), transactionId),
        }
    }
