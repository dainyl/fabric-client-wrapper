...
import { isFcwChannel } from './fabric-client-extended'
...
export default class Transactor {
    ...
    */
    async registerChaincodeEventListener(eventName: string, onEvent: Function, onError?: Function) {
        if (!isFcwChannel(this.channel)) {
            throw new Error('Error Transactor.registerChaincodeEvent requires a FcwChannel')
        }
        const eventHubManager = this.channel.getPeers()[0].getEventHubManager()
        const handle = await eventHubManager.registerChaincodeEvent(
            this.chaincodeId,
            eventName,
            event => {
                if (this.channel.getTransactionIdTimeMap().has(event.tx_id)) {
                    onEvent(event.payload.toString())
                }
            },
            onError
        )
        return () => eventHubManager.unregisterChaincodeEvent(handle)
    }
}
