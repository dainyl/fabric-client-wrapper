...
import { isFcwPeer, upgradePeerToFcwPeer, createFcwPeer, type FcwPeerOpts, type CreateFcwPeerOpts } from './FcwPeer'
...
export function upgradeFcwPeerToEventHubPeer(fcwPeer: Peer, { client, eventUrl, eventHubOpts }: EventHubPeerOpts) {
    ...
    fcwPeer.canConnect = (timeout = 60000) =>
        new Promise(resolve => {
            ...
            const handle = setTimeout(() => {
                fcwPeer.eventHubManager.unregisterBlockEvent(blockRegistrationNumber)
                resolve(false)
            }, timeout)
...
export function upgradePeerToEventHubPeer(peer: Peer, eventHubPeerOpts: EventHubPeerOpts) {
    return upgradeFcwPeerToEventHubPeer(upgradePeerToFcwPeer(peer, eventHubPeerOpts), eventHubPeerOpts)
}

export function createEventHubPeer(createEventHubPeerOpts: CreateEventHubPeerOpts) {
